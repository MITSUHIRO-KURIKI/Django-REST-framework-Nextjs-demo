...
def get_ai_icon_image_path(instance, filename):
    return f'frontend/apps/llmchat/ai_icon/{instance.room_id.pk}/{filename}'
...
class Room(models.Model):
    ...
    create_user = models.ForeignKey(
                    ...
                    blank        = False,
                    null         = False,
                    related_name = 'related_llmchat_room_model_create_user',
...
class RoomSettings(models.Model):
    ...
    room_id = models.OneToOneField(
                    ...
                    blank        = False,
                    null         = False,
                    related_name = 'related_llmchat_room_settings_model_room_id',
                    help_text    = '紐づくルームID',)
    ...
    ai_icon = models.ImageField(
                    verbose_name = _('AIアイコン'),
                    upload_to    = get_ai_icon_image_path,
                    blank        = True,
                    null         = True,
                    default      = '',
                    help_text    = str(_('画像は {AI_ICON_RESIZE_HEIGHT} (px)x {AI_ICON_RESIZE_WIDTH} (px) にリサイズされます').format(AI_ICON_RESIZE_HEIGHT=AI_ICON_RESIZE_HEIGHT, AI_ICON_RESIZE_WIDTH=AI_ICON_RESIZE_WIDTH)),)
    ...
    def save(self, *args, **kwargs):
        ...
        # user_icon 画像のリサイズ処理
        try:
            resize_width  = AI_ICON_RESIZE_HEIGHT
            resize_height = AI_ICON_RESIZE_HEIGHT
            if self.ai_icon.width > resize_width or self.ai_icon.height > resize_height:
                
                new_width  = resize_width
                new_height = resize_height
                resized    = get_thumbnail(self.ai_icon, f'{new_width}x{new_height}',
                                           crop    = 'center',
                                           quality = 99)
                name       = self.ai_icon.name.split('/')[-1]
                
                self.ai_icon.save(name, ContentFile(resized.read()), True)
                delete(resized) # キャッシュファイルの削除
        except: pass            # ai_icon が無い場合には処理回避

    class Meta:
        app_label    = 'llmchat'
        db_table     = 'llmchat_room_settings_model'
